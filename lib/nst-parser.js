	...
	parse: function(o, src){
		...
		do{
			...
			for(var i in this.grammer.marks){
				...
				m = re.exec(s);
				if(!m) continue;
				e = {text:s, label:"", line: this.line_idx, position: this.position, type: def.t};
				e.label = (m[1]) ? m[1] : m[0];
...
parser.langs['JavaScript'] = {
	...
	marks: [
		...
		{t:'obj', p: 'name = {'},
		{t:'obj', p: 'name : {'},
		{t:'obj', p: '*name = \\(function() {'},
		{t:'obj', re: /^\s*([a-zA-z0-9\_\.]+)\s*(?=\[).+\]\s*\=\s*\{/ },

		{t:'proto', p: 'id.prototype.name = function()'},
		{t:'proto', p: 'name.prototype = {'},

		{t:'jq', re: /^\s*\$\.fn\.([a-zA-z0-9\_]+)\s*\=\s*function(?=\().+\)\s*\{/},
		{t:'jq', re: /^\s*jQuery\.fn\.([a-zA-z0-9\_]+)\s*\=\s*function(?=\().+\)\s*\{/},

		{t:'fn', p: '\\(function() {', o :{"text": "IIFE", class:['nst-italic']}},

        {t:'fn', p: 'function name() {', o: {class:'nst-icn-function'}},
        {t:'fn', p: '*name = function() {', o: {class:'nst-icn-function'}},
        {t:'fn', p: 'name : function() {', o: {class:'nst-icn-function'}},
...
parser.langs['PHP'] = {
	...
	marks: [
		...
		{t:'obj', re: /^\s*class\s+([a-zA-z0-9\_]+)\s*/},
		{t:'obj', re: /^\s*trait\s+([a-zA-z0-9\_]+)\s*/},
		{t:'obj', re: /^\s*interface\s+([a-zA-z0-9\_]+)\s*/, o :{class:['nst-italic']} },

		{t:'fn', re: /^\s*(\$[.a-zA-Z0-9_$]*)\s*\=\s*function\s*(?=\().+\)\s*\{/},
        {t:'fn', re: /^\s*private\s+function\s*([a-zA-z0-9\_]+)\s*(?=\().+\)/},
		{t:'fn', re: /^\s*public\s+function\s*([a-zA-z0-9\_]+)\s*(?=\().+\)/},
		{t:'fn', re: /^\s*function\s*([a-zA-z0-9\_]+)\s*(?=\().+\)/},
		{t:'fn', re: /^\s*private\s+static\s+function\s*([a-zA-z0-9\_]+)\s*(?=\().+\)/},
		{t:'fn', re: /^\s*public\s+static\s+function\s*([a-zA-z0-9\_]+)\s*(?=\().+\)/},
	],
	re: [
		{m: /\./g, p: '\\.'},
		{m: /\*/g, p: '.*?'},
