...
class NstView {
	...
	filterList(text){
		...
		if(!this.currentState || !this.currentState.list) return;

		this.clear();
		this.populateChilds(this.root, this.currentState.list.items, {text:text});
	}
	populateList(state){
		if(state){
		...
		this.clear();
		if(!this.currentState || !this.currentState.list) return;
		this.populateChilds(this.root, this.currentState.list.items, {parentID:0});
	}
	...
	pupulateMarks(){
		...
		var state = parser.parse(this, ed.getText());
		this.populateList(state);
	}
